...
I’ve often seen Node.js cache code written like this:
...
This example is using [Redis](https://redis.io/)'s [node library](https://github.com/NodeRedis/node-redis) and SQL of some flavor such as [PostgreSQL](https://www.npmjs.com/package/pg). `'EX', '3600'` in our `redisClient.set()` tells tells Redis our key has a time to live (TTL) of 1 hour.
...
User input can also lead to cascading database calls. One of the more interesting performance bug I’ve seen in my career was while I was contracting with Microsoft, which I'll get to in a moment. 
...
This looks more complicated than it is, juggling promises without async/await adds a lot of boilerplate but if you take a look at the insides its rather straightforward. The first call to batch for a key will call promiseFunction and await for it to resolve with a value. Any calls to batch in the meanwhile with the same key will also wait on and resolve when that first promiseFunction resolves.
